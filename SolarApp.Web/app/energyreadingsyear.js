...
d3.json("/Report/YearGraphData?targetDate=" + targetDate, function (error, json) {
...
function visualizeit() {
    ...
    var yRange1 = d3.time.scale.utc().range([HEIGHT - MARGINS.top, MARGINS.bottom]).domain([0,
        d3.max(dataArray, function (d) { return d.monthEnergy; })]);
    ...
    svg.append("text")
        ...
        .attr("x", WIDTH * 0.5)
        .attr("y", 10)
        .text("Graph of production for " + moment(targetDate).format("YYYY"));
